...
check_pointer(struct client *client, int x, int y)
{
	...
	g_assert(client->test->pointer_x == x);
	g_assert(client->test->pointer_y == y);
	...
	if (surface_contains(client->surface, x, y)) {
		...
		g_assert(client->input->pointer->focus == client->surface);
		 ...
		 */
		g_assert(client->input->pointer->focus == NULL);
	}
}
...
MUTTER_TEST(test_pointer_top_left)
{
	...
	g_assert(!surface_contains(client->surface, x, y));
...
MUTTER_TEST(test_pointer_bottom_left)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_top_right)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_bottom_right)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_top_center)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_bottom_center)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_left_center)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_right_center)
{
	struct client *client;
	int x, y;
...
MUTTER_TEST(test_pointer_surface_move)
{
	struct client *client;
	...
	g_assert(!surface_contains(client->surface, 50, 50));
...
check_client_move(struct client *client, int x, int y)
{
	...
	if (output_contains_client(client)) {
		g_assert(client->surface->output == client->output);
	} else {
		g_assert(client->surface->output == NULL);
	}
}

MUTTER_TEST(test_surface_output)
{
	struct client *client;
	int x, y;
	...
	g_assert(output_contains_client(client));
...
MUTTER_TEST(buffer_release)
{
	...
	g_assert(buf1_released == 0);
	/* buf2 may or may not be released */
	g_assert(buf3_released == 0);
