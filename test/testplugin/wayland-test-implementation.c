              ...
              int32_t y)
{
  ...
  /* It's possible that the movement was constrained. Check the result */
  meta_window_get_frame_rect (surf->window, &rect);
  if (x != rect.x || y != rect.y)
    g_warning("Window movement was constrained. Current position:"
              " %dx%d, desired position: %dx%d", rect.x, rect.y, x, y);
}
              ...
              int32_t y)
{
  ...
  clutter_event_set_stage (ev, stage);
  ClutterActor *actor =
          ...
          uint32_t state)
{
  ...
  clutter_event_set_stage (ev, get_stage ());
  clutter_event_set_time (ev, get_time ());
